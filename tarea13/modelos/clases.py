from tarea13.config.mysqlconnection import conectarMySQL
from flask import flash
import re	# el módulo regex - # crea un objeto de expresión regular que usaremos más adelante


EMAIL_REGEX = re.compile(r'^[a-zA-Z0-9.+_-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]+$')
CONTRASENA_REGEX = re.compile(r"^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{6,}$")
#EMAIL_REGEX: es una variable de python
...
class Usuarios:
    ...
    def obtener_todo(cls):
        ...
        results = conectarMySQL('registro_schema').query_db(query) #result serian un diccionario en donde conectamos con el nombre de nuestra base de datos y  vamos a llamar a la función conectarMySQL con el esquema al que te diriges
...
#METODO CREATE con INSERT
    ...
    def registro_usuario(cls, data): #(nombre de las columnas en nuestra tabla y en VALUES nombre de las claves de nuestro diccionario del controlador de forma sanitizada)
        ...
        return conectarMySQL('registro_schema').query_db(query, data)
...
#para obtener un usuario a traves de su id
    ...
    def obtener_un_usuario(cls, data):
        ...
        results =  conectarMySQL('registro_schema').query_db(query, data)
...
#a traves del email estamos obteniendo un usuario
    ...
    def obtener_por_email(cls,data):
        ...
        result = conectarMySQL('registro_schema').query_db(query, data)
    ...
    def validar_formulario(usuario):
        ...
        if (usuario['fnacimiento']):
            ...
            validar = False
       # if len(usuario['contraseña']) < 9:
        #    flash("ATENCIÓN ¡CONTRASEÑA debe tener al menos 8 caracteres!") 
           # validar = False
        ...
        if (usuario['ubicacion']) == "":
            ...
            validar = False
        if not EMAIL_REGEX.match(datos_email['email']): #aca validamos el mail #antes lo validamos match(usuario['email'])
            flash("ATENCIÓN ¡Direccion de EMAIL no es valido!") #enviamos el valor de la clave datos_email 
            validar = False
        elif Usuarios.obtener_por_email(datos_email): #elif esta conmectado a la pregunta de email, consultamos si existe en la bd
